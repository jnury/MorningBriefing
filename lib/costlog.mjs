...
function usageLine(label, entry) {
  const u = entry.usage ?? emptyUsage(); // defensive: this module's whole purpose is robust logging
...
// `consumers` (edition ids sharing this bucket, from lib/plan.mjs) is what
// makes "which editions did this bucket serve, and what did it cost"
// answerable from the JSONL alone — the cost itself is the shared collection
// cost, not split per edition, since collection genuinely happens once.
const flattenBucket = (b) => ({
  id: b.id, kind: b.kind, consumers: b.consumers ?? [], ok: b.ok, durationMs: b.durationMs ?? null, ...b.usage,
});
