...
async function collectOne(bucket, ctx) {
  ...
  const validated = validateBucket(data, topic, ctx.date);
  if (!validated.valid) {
    const err = new Error(`vivier invalide :\n - ${validated.errors.join('\n - ')}`);
    err.usage = usage; throw err;
  }
  // Dropped candidates are normal attrition, but silent attrition hides a
  // drifting collector (a prompt the model keeps overshooting) until the day it
  // drops everything, so each one is named in the log.
  for (const d of validated.dropped) {
    ctx.log?.(`collecte: ${bucket.id} — élément ${d.index} écarté (${d.errors.join(' ; ')})`);
  }
  // The filtered bucket, never the raw one: composeEdition's invariant is that
  // every item it can select has already passed validation.
  data = validated.data;
  ...
  //
  // A researched bucket is rewritten only when validation dropped something,
  // because the selection prompt points the editor at this file: leaving a
  // rejected candidate in it lets the editor spend one of its slots on an item
  // compose is bound to discard, returning a short section. Rewriting also
  // makes --recompose idempotent — the reloaded pool drops nothing a second
  // time. The rejected candidates survive in the log, which names each one.
  if (topic.kind === 'provider' || validated.dropped.length > 0) {
