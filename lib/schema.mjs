...
// Bucket-side counterpart to checkItems, and the reason the two contracts are
// not symmetric. A bucket is an over-collected candidate pool (OVERCOLLECT in
// plan.mjs deliberately researches ~1.5x what any edition consumes, because the
// freshness filter is expected to discard candidates), so a malformed candidate
// is an ordinary event that must cost exactly one candidate. Failing the pool
// wholesale is what erased a 33-item tech bucket from both editions on
// 2026-08-03 over a summary 4 words past its cap.
//
// An empty pool stays fatal: there is nothing to select from. When everything
// is dropped, the per-item reasons are folded into the fatal message, otherwise
// a wholesale failure reports only "no valid items" and is undiagnosable.
function filterItems(where, items, topic, editionDate) {
  if (!Array.isArray(items) || items.length === 0) {
    return { kept: [], dropped: [], fatal: [`${where}.items doit contenir au moins un élément`] };
  }
  ...
  items.forEach((item, i) => {
    const itemErrors = [];
    checkItem(itemErrors, `${where}.items[${i}]`, item, topic, editionDate);
    if (itemErrors.length === 0) kept.push(item);
    else dropped.push({ index: i, errors: itemErrors });
  });

  if (kept.length === 0) {
    const why = dropped.flatMap((d) => d.errors).join(' ; ');
    return { kept, dropped, fatal: [`${where}.items: aucun élément valide — ${why}`] };
  }
  return { kept, dropped, fatal: null };
}
...
//
// Returns { valid, errors, data, dropped }. `data` is the bucket the caller
// should use: for a researched topic it is the bucket minus any candidate that
// failed item-level validation, so callers MUST propagate it rather than reuse
// the input. `dropped` lists those rejected candidates ({ index, errors }) for
// logging — it is populated whether or not the bucket is valid. `errors` holds
// only fatal problems, i.e. the reasons `valid` is false.
export function validateBucket(bucket, topic, editionDate) {
  const errors = [];
  if (!bucket || typeof bucket !== 'object') {
    return { valid: false, errors: ['bucket: objet attendu'], data: bucket, dropped: [] };
  }
  ...
  // Only a researched pool tolerates partial loss. A provider or dataset bucket
  // is a precise reading, not a pool of candidates: a bad city or index there
  // means the source is wrong, so it stays all-or-nothing.
  let data = bucket;
  let dropped = [];
  if (topic.kind === 'provider') checkCities(errors, where, bucket.cities);
  else if (topic.kind === 'dataset') checkMarkets(errors, where, bucket);
  else {
    const filtered = filterItems(where, bucket.items, topic, editionDate);
    dropped = filtered.dropped;
    if (filtered.fatal) errors.push(...filtered.fatal);
    else data = { ...bucket, items: filtered.kept };
  }

  return { valid: errors.length === 0, errors, data, dropped };
}
