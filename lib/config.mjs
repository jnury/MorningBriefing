...
// `label` ('topic' | 'édition') matches the prefix used by checkTopic/checkEdition
// messages below. A malformed file is collected as an error, not thrown — one
// broken file must not stop validation of the rest.
function readJsonDir(dir, label, errors) {
  let files = [];
  ...
  for (const f of files.filter((f) => f.endsWith('.json')).sort()) {
    ...
    try {
      out.push({ name, data: JSON.parse(readFileSync(join(dir, f), 'utf8')) });
    } catch {
      errors.push(`${label} ${name}: JSON invalide dans ${f}`);
    }
  }
  return out;
}
...
function checkTopic(errors, name, t) {
  ...
  if (t.kind === 'topic') {
    ...
    // Optional, but meaningless unless it leaves headroom under the cap.
    if (t.summaryTargetWords !== undefined) {
      if (!isNum(t.summaryTargetWords)) {
        errors.push(`topic ${name}: summaryTargetWords doit être un nombre`);
      } else if (isNum(t.summaryMaxWords) && t.summaryTargetWords > t.summaryMaxWords) {
        errors.push(`topic ${name}: summaryTargetWords (${t.summaryTargetWords}) dépasse summaryMaxWords (${t.summaryMaxWords})`);
      }
    }
  }
...
export function loadConfig(root) {
  ...
  const topics = {};
  for (const { name, data } of readJsonDir(join(dir, 'topics'), 'topic', errors)) {
  ...
  const editions = [];
  for (const { name, data } of readJsonDir(join(dir, 'editions'), 'édition', errors)) {
